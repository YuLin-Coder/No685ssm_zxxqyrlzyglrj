...
<body>
	<nav class="breadcrumb">
		...
		求职简历查询 <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px"
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="6">求职简历查询</th>
				</tr>
				<tr class="text-c">
					<th class="center">求职单号</th>
					<th class="center">招聘信息</th>
					<th class="center">简历文件</th>
			...
			<c:forEach items="${applysList}" var="applys">
				<tr class="text-c">
					<td class="center">${applys.ano}</td>
					<td class="center">${applys.title}</td>
					<td class="center"><a href="${applys.filurl}">下载简历</a></td>
		...
		<div class="text-c">
			<form action="applys/queryApplysByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						<option value="ano">按求职单号查询</option>
						<option value="jobsid">按招聘信息查询</option>
						<option value="filurl">按简历文件查询</option>
						<option value="addtime">按报名日期查询</option>
