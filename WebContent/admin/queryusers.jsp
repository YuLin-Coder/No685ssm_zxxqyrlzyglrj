...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>职工账户管理 <span class="c-gray en">&gt;</span>
		职工账户查询 <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px"
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="17">职工账户查询</th>
				</tr>
				<tr class="text-c">
			...
			<c:forEach items="${usersList}" var="users">
				<tr class="text-c">
					...
					<td class="center">${users.deptname}</td>
					<td class="center">${users.dutyname}</td>
					<td class="center">${users.graduate}</td>
					<td class="center">${users.xueli}</td>
					<td class="center">${users.idcard}</td>
					<td class="center">${users.contact}</td>
					<td class="center">${users.address}</td>
					<td class="center">${users.workdate}</td>
		...
		<div class="text-c">
			<form action="users/queryUsersByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						...
						<option value="image">按头像查询</option>
