...
<body>
	<nav class="breadcrumb">
		...
		员工培训查询 <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px"
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="8">员工培训查询</th>
				</tr>
				<tr class="text-c">
					<th class="center">部门</th>
					<th class="center">员工</th>
					<th class="center">培训类型</th>
					<th class="center">培训原因</th>
			...
			<c:forEach items="${trainingList}" var="training">
				<tr class="text-c">
					<td class="center">${training.deptname}</td>
					<td class="center">${training.realname}</td>
					<td class="center">${training.cate}</td>
					<td class="center">${training.reason}</td>
					<td class="center">${training.thestart}</td>
					<td class="center">${training.theend}</td>
					<td class="center">${training.memo}</td>
				</tr>
			</c:forEach>
		</table>
		<div class="cl pd-5 mt-20"></div>
		<div class="text-c">
			<form action="training/queryTrainingByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						...
						<option value="cate">按培训类型查询</option>
						<option value="reason">按培训原因查询</option>
